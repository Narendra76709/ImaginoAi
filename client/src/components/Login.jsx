    ...
    try {
      if (state === 'Login') {
        ...
        if (data.success) {
          setToken(data.token);
          setUser(data.user);
          localStorage.setItem('token', data.token); // ✅ CORRECTED
          setShowLogin(false);
        } else {
      ...
      >
        ...
        {state !== 'Login' && (
          <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-5'>
            <img src={assets.profile_icon} className='h-6' alt='Profile icon' />
            <input
        ...
        <button type='submit' className='text-white bg-blue-600 w-full py-2 rounded-full'>
